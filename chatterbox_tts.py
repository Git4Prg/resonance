...
S3_BUCKET_NAME = "resonance-app-voices"
STORAGE_MOUNT_PATH = "/storage"
storage_bucket = modal.CloudBucketMount(
    S3_BUCKET_NAME,
    secret=modal.Secret.from_name("aws-s3-credentials"),
    read_only=True,
)
...
@app.cls(
    ...
    volumes={STORAGE_MOUNT_PATH: storage_bucket},
)
...
class Chatterbox:
    ...
    def serve(self):
        ...
        def generate_speech(request: TTSRequest):
            voice_path = Path(STORAGE_MOUNT_PATH) / request.voice_key
...
def test(
    ...
    audio_prompt_path = f"{STORAGE_MOUNT_PATH}/{voice_key}"
